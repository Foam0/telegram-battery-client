...
**Prerequisites:** Android SDK with NDK 23.2.8568313, JDK 21, and [Ninja](https://ninja-build.org/).

Clone the repository (submodules are initialized automatically at build time):

```
git clone https://github.com/drizzt/Mercurygram.git
```
...
Native libraries (FFmpeg, BoringSSL, libvpx, dav1d) are built from source automatically on the first build and cached for subsequent runs.
