...
selected. Because the VAPID keypair belongs to the gateway, this entry always
uses the Mercurygram gateway, ignoring a custom Gateway URL.
...
**Prerequisites:** Android SDK with the NDK version pinned by `ndkVersion` in `TMessagesProj/build.gradle`, JDK 17, `git`, and the native toolchain the media libraries need: [Ninja](https://ninja-build.org/), [Meson](https://mesonbuild.com/) (dav1d), `nasm` (x86 assembly in libvpx and ffmpeg), `cmake` and `pkg-config`. `curl` is needed too: the build installs the pinned Rust toolchain that `jni/tlottie_lib/build.sh` uses (see `TMessagesProj/jni/setup_rust.sh`), because upstream ships the tlottie archives prebuilt and Mercurygram builds them from source instead.
