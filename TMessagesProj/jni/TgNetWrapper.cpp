...
static JNIEnv *getCallbackJNIEnv() {
    JNIEnv *env = nullptr;
    if (java == nullptr) {
        return nullptr;
    }
...
void sendRequest(JNIEnv *env, jclass c, jint instanceNum, jlong object, jint flags, jint datacenterId, jint connectionType, jboolean immediate, jint token) {
    ...
    ConnectionsManager::getInstance(instanceNum).sendRequest(request, ([instanceNum, token](TLObject *response, TL_error *error, int32_t networkType, int64_t responseTime, int64_t msgId, int32_t dcId) {
        ...
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onRequestQuickAck, instanceNum, token);
    }), ([instanceNum, token] {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onRequestWriteToSocket, instanceNum, token);
    }), ([instanceNum, token] {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onRequestClear, instanceNum, token, false);
...
void cancelRequest(JNIEnv *env, jclass c, jint instanceNum, jint token, jboolean notifyServer) {
    return ConnectionsManager::getInstance(instanceNum).cancelRequest(token, notifyServer, ([instanceNum, token]() -> void {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onRequestClear, instanceNum, token, true);
    }));
}
...
jlong checkProxy(JNIEnv *env, jclass c, jint instanceNum, jstring address, jint port, jstring username, jstring password, jstring secret, jobject requestTimeFunc) {
    ...
    jlong result = ConnectionsManager::getInstance(instanceNum).checkProxy(addressStr, (uint16_t) port, usernameStr, passwordStr, secretStr, [instanceNum, requestTimeFunc](int64_t time) {
        if (requestTimeFunc != nullptr) {
            JNIEnv *env = getCallbackJNIEnv();
            if (env != nullptr) env->CallVoidMethod(requestTimeFunc, jclass_RequestTimeDelegate_run, time);
        }
    }, requestTimeFunc);
...
class Delegate : public ConnectiosManagerDelegate {
    ...
    void onUpdate(int32_t instanceNum) {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onUpdate, instanceNum);
    }
    
    void onSessionCreated(int32_t instanceNum) {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onSessionCreated, instanceNum);
    }
    
    void onConnectionStateChanged(ConnectionState state, int32_t instanceNum) {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onConnectionStateChanged, state, instanceNum);
    }
    ...
    void onUnparsedMessageReceived(int64_t reqMessageId, NativeByteBuffer *buffer, ConnectionType connectionType, int32_t instanceNum) {
        if (connectionType == ConnectionTypeGeneric) {
            JNIEnv *env = getCallbackJNIEnv();
            if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onUnparsedMessageReceived, (jlong) (intptr_t) buffer, instanceNum, reqMessageId);
        }
    }
    
    void onLogout(int32_t instanceNum) {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onLogout, instanceNum);
    }

    void onReducedTempKeyExhausted(int32_t instanceNum) {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onReducedTempKeyExhausted, instanceNum);
    }
    ...
    void onInternalPushReceived(int32_t instanceNum) {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onInternalPushReceived, instanceNum);
    }

    void onBytesReceived(int32_t amount, int32_t networkType, int32_t instanceNum) {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onBytesReceived, amount, networkType, instanceNum);
    }

    void onBytesSent(int32_t amount, int32_t networkType, int32_t instanceNum) {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onBytesSent, amount, networkType, instanceNum);
    }

    void onRequestNewServerIpAndPort(int32_t second, int32_t instanceNum) {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onRequestNewServerIpAndPort, second, instanceNum);
    }

    void onProxyError(int32_t instanceNum) {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onProxyError);
    }
    ...
    int32_t getInitFlags(int32_t instanceNum) {
        JNIEnv *env = getCallbackJNIEnv();
        return env != nullptr ? (int32_t) env->CallStaticIntMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_getInitFlags) : 0;
    }

    void onPremiumFloodWait(int32_t instanceNum, int32_t requestToken, bool isUpload) {
        JNIEnv *env = getCallbackJNIEnv();
        if (env != nullptr) env->CallStaticVoidMethod(jclass_ConnectionsManager, jclass_ConnectionsManager_onPremiumFloodWait, instanceNum, requestToken, isUpload);
    }
