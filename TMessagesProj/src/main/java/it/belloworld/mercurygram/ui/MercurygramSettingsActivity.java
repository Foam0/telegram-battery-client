...
public class MercurygramSettingsActivity extends UniversalFragment {
    ...
    protected void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        if (!MgUpdateChecker.isFdroidBuild()) {
            ...
            if (!MgUpdateChecker.isBetaChannel()) {
                boolean onPreSource = MgUpdateChecker.isOnPreSourceInstall();
    ...
    private void handleAcceptPreReleasesClick() {
        // Pre-source installs (X.Y.Z.0.K) carry a higher MG_VC than any prior
        // stable, so PackageInstaller refuses the rollback — keep the gate.
        // Post-stable betas (X.Y.Z.W.K, W>=1) share MG_VC with X.Y.Z.W and can
        // be rolled back; toggling off triggers the download of that stable.
        if (MgUpdateChecker.isOnPreSourceInstall()) {
            return;
        }
