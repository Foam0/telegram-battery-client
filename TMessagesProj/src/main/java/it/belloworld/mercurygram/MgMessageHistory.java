 ...
 * feature is opt-in (per-account {@code UserConfig.savedMessagesHistory}, default off).</p>
 */
public class MgMessageHistory {
    ...
    public void archiveEditsBefore(int account, long dialogId, ArrayList<TLRPC.Message> newMessages) {
        if (!UserConfig.getInstance(account).savedMessagesHistory || newMessages == null || newMessages.isEmpty()) {
            return;
        }
    ...
    private void archive(int account, long dialogId, ArrayList<Integer> mids, boolean deleted) {
        if (!UserConfig.getInstance(account).savedMessagesHistory || mids == null || mids.isEmpty()
