...
public class MgMessageHistory {
    ...
    /** Cheap existence check for the deleted-history chat-input icon badge. */
    public boolean hasEntries(int account, long dialogId) {
        synchronized (writeLock) {
            try {
                android.database.sqlite.SQLiteDatabase db = dbHelper.getReadableDatabase();
                return existsIn(db, account, dialogId, TBL_DELETED)
                        || existsIn(db, account, dialogId, TBL_EDITED);
            } catch (Exception e) {
                FileLog.e(e);
                return false;
            }
    ...
    private boolean existsIn(android.database.sqlite.SQLiteDatabase db,
                             int account, long dialogId, String table) {
        Cursor c = null;
        try {
            c = db.rawQuery("SELECT 1 FROM " + table
                            + " WHERE account=? AND dialog_id=? LIMIT 1",
