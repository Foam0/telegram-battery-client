...
public class MgUpdateChecker {
    ...
    // Runtime versionName/versionCode aliases the stable release for both
    // beta.yml snapshots and pre-source builds, so the only reliable signal is
    // the tag the in-app updater last installed.
    public static boolean isOnPreReleaseInstall() {
        String tag = SharedConfig.mgLastInstalledTag;
        return tag != null && tag.split("\\.", -1).length >= 5;
    }

    // Pre-source tags (X.Y.Z.0.K) carry a higher MG_VC than any prior stable,
    // so Android refuses the downgrade install — the toggle stays locked here.
    // Post-stable beta tags (X.Y.Z.W.K, W>=1) share MG_VC with the X.Y.Z.W
    // stable and can be rolled back via PackageInstaller.
    public static boolean isOnPreSourceInstall() {
        String tag = SharedConfig.mgLastInstalledTag;
        if (tag == null) return false;
        String[] p = tag.split("\\.", -1);
        return p.length == 5 && "0".equals(p[3]);
    }
    ...
    // Rolls back from a 5-dotted Release-flavor beta (X.Y.Z.W.K, W>=1) to the
    // X.Y.Z.W stable. Both share MG_VC so PackageInstaller accepts the install.
    // Pre-source tags (X.Y.Z.0.K) are ineligible (MG_VC sits above any prior
    // stable, OS refuses the downgrade) and short-circuit here.
    public static void checkForDowngradeToStable() {
        String targetTag = derivePrecedingStableTag(SharedConfig.mgLastInstalledTag);
    ...
    public static void downloadUpdate(MgUpdateInfo info, ProgressCallback callback) {
        ...
        Utilities.globalQueue.postRunnable(() -> {
            ...
            try {
                ...
                SharedConfig.mgLastInstalledTag = info.tagName;
                SharedConfig.saveConfig();
    ...
    private static String currentInstallVersion() {
        String tag = SharedConfig.mgLastInstalledTag;
    ...
    // chronology falls out of plain numeric vector compare because a 4-part
    // stable padded with 0 sits between 5-part pre-source (X4=0) and 5-part
    // snapshot (X4>=1) of the same base.
    static boolean versionUpToDate(String current, String tag) {
        ...
        int len = Math.max(c.length, t.length);
        for (int i = 0; i < len; i++) {
            long a = i < c.length ? c[i] : 0;
            long b = i < t.length ? t[i] : 0;
            if (a != b) return a > b;
        }
        return true;
    }
