...
PY
  ...
  verify-beta)
    ...
    podman run --rm --network=host -v "$work/src":/src:z -w /src "$IMG" \
        bash -euo pipefail -c "
            # Install Java 17 from bookworm (matches the recipe sudo: block;
            # GitHub Actions uses temurin-17 — same major version, build
            # output is deterministic across vendor d8/r8 given same inputs).
            ...
            ./gradlew :TMessagesProj_App:assemble${flavor}Debug --no-daemon \
                ...
                -PRELEASE_STORE_PASSWORD=stub -PRELEASE_KEY_PASSWORD=stub -PRELEASE_KEY_ALIAS=stub
    ...
    log "diffoscope github.apk vs local.apk (signatures excluded)"
    if diffoscope_apks github.apk local.apk; then
        log "RESULT: REPRODUCIBLE — GitHub beta APK matches local F-Droid-env build (excluding signatures)"
        exit 0
    fi
