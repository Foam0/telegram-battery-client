...
override_recipe_keep_flavor() {
    local snap_sha="$1"
    local MG_VC_BASE
    # `|| true` so grep returning 1 (no match) doesn't trip set -e + pipefail
    # before the explicit die guard below — the guard's message is the
    # user-facing diagnostic; without it the script exits silently.
    MG_VC_BASE=$(grep -oE '^MG_VERSION_CODE=[0-9]+' \
        "$work/src/gradle.properties" | cut -d= -f2 || true)
    [ -n "$MG_VC_BASE" ] || die "could not parse MG_VERSION_CODE from snapshot"
    podman run --rm -i -v "$work":/w:z "$IMG" python3 - \
        "/w/fdroiddata/metadata/${APPID}.yml" "/w/fdroiddata/${APPID}.git" \
        "$snap_sha" "$NDK_VERSION" "$MG_VC_BASE" <<'PY'
import sys, yaml
recipe, bundle, sha, ndk, vc_base = sys.argv[1:6]
FLAVOR_OFFSETS = {
...
override_recipe_for_abi() {
    ...
    podman run --rm -i -v "$work":/w:z "$IMG" python3 - \
        "/w/fdroiddata/metadata/${APPID}.yml" "/w/fdroiddata/${APPID}.git" \
        "$snap_sha" "$NDK_VERSION" "$MG_VC_BASE" "$abi" <<'PY'
import sys, yaml
recipe, bundle, sha, ndk, vc_base, abi = sys.argv[1:7]
ABI_FLAVOR = {
    'x86':         ('afatFdX86', 3),
...
override_recipe_with_vc() {
    ...
    podman run --rm -i -v "$work":/w:z "$IMG" python3 - \
        "/w/fdroiddata/metadata/${APPID}.yml" "/w/fdroiddata/${APPID}.git" \
        "$snap_sha" "$vc" "$NDK_VERSION" <<'PY'
import sys, yaml
recipe, bundle, sha, vc, ndk = sys.argv[1], sys.argv[2], sys.argv[3], int(sys.argv[4]), sys.argv[5]
with open(recipe) as f:
